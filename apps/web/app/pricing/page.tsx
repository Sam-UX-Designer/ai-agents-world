  ...
  return (
    <main className="pricing">
      <header className="pricing__head">
        ...
        <p>
          Every plan runs the same world and the same agents. What changes is how
          many goals you get, and how hard the agents think.
        </p>
      ...
      <section className="pricing__packs" aria-label="Credit packs">
        ...
        <p>
          Top-ups work on any paid plan. One credit is one goal, they never
          expire, and there is no subscription attached.
        </p>
        <ul>
          {TOP_UP_PACKS.map((pack) => {
            ...
            return (
              <li key={pack.key}>
                <strong>{pack.goals.toLocaleString()} goals</strong>
                <em>${pack.priceUsd}</em>
                {saving > 0 && <span className="pricing__save">Save {saving}%</span>}
                <button className="btn btn--ghost" onClick={() => setAsked(`${pack.goals} goals`)}>
                  Add
                </button>
              </li>
            )
