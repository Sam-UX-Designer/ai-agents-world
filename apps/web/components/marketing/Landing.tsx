 ...
 * The landing page.
 *
 * What it is for: someone arrives from the main site knowing nothing except a
 * product name. In one scroll they should understand what this is, why it
 * exists, and what it would do for them, and then be able to start.
 *
 * Every image here is a crop of the supplied island render. That is a
 * deliberate constraint rather than a shortage: the island is the product's
 * one real visual, and a marketing page that showed anything else would be
 * selling a different thing. Nothing here is generated, and there are no
 * mocked-up screenshots.
 *
 * The copy rule throughout is one idea per section, said once, in the
 * shortest true sentence. If a paragraph needed a second paragraph to make
 * sense, the first one was wrong.
 */
...
function Nav() {
  return (
    <nav className="lp__nav">
      <Link href="/" className="lp__brand" aria-label="AI Agents World, home">
        <Logo />
        <span>AI Agents World</span>
      </Link>
      <div className="lp__navlinks">
        <Link href="/pricing">Plans</Link>
        <Link href="/signin" className="lp__btn lp__btn--small">
          Get started
        </Link>
      </div>
    </nav>
...
/**
 * The island, full bleed, with the promise over it.
 *
  ...
  return (
    <header className="lp__hero">
      <div className="lp__heroart">
        <WorldArt src={ISLAND} className="lp__heroimg" />
      </div>
      ...
      <div className="lp__herotext">
        ...
        <p>
          You give one goal. It is split into tasks, handed to the specialists
          that fit, and comes back as one answer.
        </p>
        <div className="lp__cta">
          <Link href="/signin" className="lp__btn">
            Get started
          </Link>
          <Link href="/pricing" className="lp__btn lp__btn--quiet">
            See plans
          </Link>
        </div>
      </div>
...
function WhatIsAnAgent() {
  return (
    <section className="lp__split">
      <Reveal className="lp__splittext">
      ...
      <Reveal className="lp__splitart" delay={0.08}>
...
function HowItWorks() {
  return (
    <section className="lp__bento">
      <Reveal className="lp__bentohead">
        <h2>One sentence in. One answer out.</h2>
      </Reveal>

      <Reveal className="lp__cell lp__cell--wide">
        <div className="lp__cellart">
          {/* The hub itself, which is where the deciding happens. */}
          <Crop at={[0.5, 0.33]} zoom={2.4} box={16 / 10} />
        </div>
        <div className="lp__celltext">
      ...
      <Reveal className="lp__cell lp__cell--tint" delay={0.06}>
      ...
      <Reveal className="lp__cell lp__cell--tint" delay={0.12}>
...
function WhyNow() {
  return (
    <section className="lp__editorial">
      <Reveal>
        <blockquote>
 ...
 *
 * The honesty line is the one thing in this page that is a claim about
...
function TheWorld() {
  return (
    <section className="lp__band">
      <Reveal className="lp__bandart">
        <WorldArt src={ISLAND} className="lp__bandimg" />
      </Reveal>
      <Reveal className="lp__bandtext" delay={0.08}>
        <h2>You can see it happening.</h2>
        <div className="lp__bandcols">
          <p>
...
function Plans() {
  return (
    <section className="lp__plans">
      <Reveal>
        <h2>Start free. No card.</h2>
        <p className="lp__planline">
        ...
        <Link href="/pricing" className="lp__link">
